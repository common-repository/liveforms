...
class Subject extends Field {
	...
	public function control_button() {
		ob_start();
		?>
		<li class="<?= $this->field_control_class; ?>" data-type="<?php echo __CLASS__ ?>" for="Subject">
			<span class="lfi lfi-name"><i class="fa fa-text-width"></i></span> Subject
			<a title="Subject" rel="Subject" class="add pull-right add-form-field" data-template='Subject' href="#"><i class="fas fa-plus-circle ttipf" title=""></i></a>
		</li>
	    <?php
    ...
    public function field_preview_html($fieldindex, $fieldid, $field_infos) {
        ob_start();
        $preview = "<input type='text' disabled='disabled' name='submitform[]' placeholder='".wplf_valueof($field_infos, "placeholder")."' class='form-control' value='' />";
